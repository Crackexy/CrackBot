...
async def _(event):
    if event.fwd_from:
        return
    await event.edit("Renaming In Process ! May Take Some Time If The File is Too Large 😅 After Rename Process Is Finished you Receive File Path ")
           ...
           # c_time = time.time()
            try:
                await borg.send_file(
                    ...
                    downloaded_file_name,
                    thumb=thumb,
                    caption="This file Has Been Uploaded Using [CrackBot](t.me/CrackBotUB)",
                    force_document=False,
                    allow_cache=False,
                    reply_to=event.message.id,
                    attributes=[
