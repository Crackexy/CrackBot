"""Reply to an image/stickes with `.mf` text on top ; text on bottom
"""
...
async def _(event):
    ...
    async with borg.conversation("@MemeCreatorBot") as bot_conv:
          ...
          if "Okay..." in response.text:
            await event.edit("```OH Fuq!! This is not an image! This will take sum tym to convert to image 😒!!!```")
            thumb = None
            ...
            await event.delete()
            await borg.send_message(event.chat_id, "`¯\_༼ ಥ ‿ ಥ ༽_/¯ Points to @Crackexy!🔥🔥`")
