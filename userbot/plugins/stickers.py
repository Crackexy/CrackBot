...
async def _(event):
    ...
    if userid == 953414679:
        ...
        packshortname = f"ItsCrackBot_{userid}_{pack}"
    ...
    if is_a_s:
        ...
        if userid == 1054081968:
            packname = f"CrackBot's Pack"
            packshortname = "CrackBot_Pack's"
        else:
            packname = f"{DEFAULTUSER}'s Crackbot Animated Vol.{pack}"
            packshortname = f"CrackBot_{userid}" # format: Uni_Borg_userid
            ...
            if response.text == FILLED_UP_DADDY:
                while response.text == FILLED_UP_DADDY:
                    ...
                    packshortname = f"Vol_{pack}_with_{userid}"
                       ...
                       # packname = f"{user.first_name}'s CrackBot Vol.{pack}"
                       # packshortname = "Vol._{pack}_Crackexy_ke_locker_me"
                   # else:
                       # packname = f"Vol._{pack}_CrackBot{userid}"
                        #packshortname = f"Vol._{pack}_CrackBot_{userid}_ns"
                    if not await stickerset_exists(bot_conv, packshortname):
                        await event.edit("**Pack No. **" + str(prevv) + "** is full! Making a new Pack 😎, Vol. **" + str(pack))
                        if is_a_s:
                            response = await silently_send_message(bot_conv, "/newanimated")
                        else:
                            response = await silently_send_message(bot_conv, "/newpack")
