...
async def _(event):
    ...
    if ".com" not in d_link:
        await event.edit("` I need a link to download something bish.`**(-__-)**")
    else:
    ...
    async with borg.conversation("@DeezLoadBot") as conv:
          try:
              ...
              await borg.send_file(event.chat_id, songh, caption="☣️**Here's the Requested Song!**☣️\nSong leached By CrackBot 😎")
              await event.delete()
          except YouBlockedUserError:
              await event.edit("**Error:** `unblock` @DeezLoadBot `and then retry!`")
