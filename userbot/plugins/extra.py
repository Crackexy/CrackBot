...
async def leave(e):
    if not e.text[0].isalpha() and e.text[0] not in ("/", "#", "@", "!"):
        await e.edit("`Me Iz Leaving Dis Group Oof!`")
        time.sleep(3)
        if '-' in str(e.chat_id):
...
async def source(e):
    if not e.text[0].isalpha() and e.text[0] not in ("/", "#", "@", "!"):
        await e.edit("https://github.com/Crackexy/CrackBot")

@borg.on(admin_cmd("readme$"))
#@register(outgoing=True, pattern="^.readme$")
async def reedme(e):
    if not e.text[0].isalpha() and e.text[0] not in ("/", "#", "@", "!"):
        await e.edit("https://github.com/Crackexy/crackbot/blob/master/README.md")
